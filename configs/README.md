...
Only canonical problem configs under `configs/benchmarks/dynamic/` and
`configs/benchmarks/quasistatic/` should be passed directly to
`python -m phast run`. Files with `manifest_type: command_manifest` or
`manifest_type: reproducibility_contract` are orchestration records; they list
commands, expected artifacts, or acceptance contracts for specialised example
modules and intentionally fail validation with an explanatory message if used
as a single problem config.
...
`acceptance:` block. This is free-form metadata for the reference result,
required artifacts, numerical/visual metrics, tolerances, and known caveats.
The solver does not enforce these tolerances during the run; it preserves them
in the resolved config and `run_lockfile.json`, and `explain-config` prints the
block so reviewers can see what a successful run is expected to demonstrate.
