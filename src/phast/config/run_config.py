...
Usage:
    python -m phast.run_config configs/benchmarks/dynamic/B1_branching_glass.yaml
...
from ..utils.io_utils import (
    init_zarr, write_zarr_snapshot, CSVHistory,
...
def _copy_mesh_provenance(mesh, output_dir: str) -> None:
    """Copy the loaded mesh into the run directory when it came from disk."""
    ...
    try:
        ...
        print(f"WARNING: could not copy mesh provenance '{mesh_path}' -> "
              f"'{dst}': {exc}", flush=True)
...
def main():
    ...
    parser.add_argument('--trajectory-format',
                        ...
                        default='zarr',
                        help='Legacy trajectory backend selector. This entry '
                             'point uses Zarr-only snapshots internally.')
    ...
    # Trajectory snapshots. ``output.h5`` / ``--h5`` is retained as a
    # deprecated config/CLI alias, but new run output is Zarr-first.
    zarr_root = None
    if cfg.output.h5:
        zarr_path = os.path.join(output_dir, 'training_data.zarr')
        zarr_root = init_zarr(zarr_path, mesh, mat)
        print(f"Zarr snapshots: {zarr_path} (--h5 is a deprecated alias)")
    ...
    # examples/timing_comparisons/datagen_half/torch/run.py so either
    ...
    for step in range(n_steps):
        ...
        # Zarr trajectory snapshot
        if zarr_root and step % snapshot_every == 0:
            strain = solver.fem.compute_strain(solver.u)
            exx, eyy, gxy = strain
            sxx, syy, sxy = solver.fem.compute_stress(
                solver.u, solver.d, strain=(exx, eyy, gxy))
            write_zarr_snapshot(
                zarr_root, step, mesh, solver.u, solver.d, psi, solver.H_elem,
    ...
    if want_plots or want_gif:
        ...
        if not os.path.exists(h5_path):
            ...
            if os.path.exists(zarr_path):
                print("[forward-viz] skipped: snapshots were written as "
                      "training_data.zarr; the inline BenchmarkPostProcessor "
                      "still reads legacy H5.")
            else:
                print("[forward-viz] skipped: no trajectory snapshots in run "
                      "dir (set output.h5: true or pass --h5; this now "
                      "writes training_data.zarr)")
        else:
            try:
