...
class SolverSettings:
    ...
    # (bit-exact backward compat). Set True to opt into the PhaFiDyn /
    # segregated "fresh d_{n+1} in corrector" ordering, which the audit
    # in reports/B7_dynamic_pipeline_audit_2026-05-08.md ranks as the
    # #1 non-energy-split candidate cause of the B7 79us-vs-COMSOL-33us
    # branching delay.
...
class OutputConfig:
    ...
    # Deprecated name kept for YAML/CLI compatibility. When true, active
    # run snapshots are written to training_data.zarr, not training_data.h5.
    h5: bool = False
    # Public spelling for the same Zarr trajectory-snapshot switch. ``h5``
    # remains the storage-compatible legacy alias.
    trajectory: bool = False
    # Snapshot cadence for the Zarr trajectory alias above.
    h5_every: int = 20
    vtu: bool = False
    vtu_every: int = 10
