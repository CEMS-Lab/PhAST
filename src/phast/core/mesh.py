...
class FEMMesh:
    ...
    def from_tensors(cls, nodes: torch.Tensor, elements: torch.Tensor,
        ...
        # Nodes — route through CPU first for MPS→float64 compatibility
        # (MPS cannot convert to float64 directly; must detach→cpu→dtype→device)
        mesh.nodes = nodes.detach().cpu().to(dtype=dtype).to(device=device)
        ...
        # Elements
        mesh.elements = elements.detach().cpu().to(dtype=torch.long).to(device=device)
    ...
    def elem_to_node(self, field_e: torch.Tensor) -> torch.Tensor:
        """Area-weighted projection: element field (E,) -> nodal field (N,).

        Numerator: scatter(field * area / 3) to each node.
        Denominator: M_scalar (= precomputed sum of area/3 per node).
        """
        if getattr(self, 'element_type', 'T3') != 'T3':
            raise NotImplementedError(
                "FEMMesh.elem_to_node currently supports T3 element fields "
                "only. Add a Q4 quadrature/nodal projection before using this "
                "path on Q4 meshes.")
