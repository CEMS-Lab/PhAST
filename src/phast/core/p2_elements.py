...
def p2_mesh_from_p1(
    ...
    midpoint_coords: list[torch.Tensor] = []
    seen: set[int] = set()
    for tri, p2_tri in zip(elems_cpu.tolist(), p2_elements_cpu.tolist()):
        for edge, mid in zip(
            ...
            midpoint_coords.append(0.5 * (nodes[a] + nodes[b]))

    if midpoint_coords:
        mids = torch.stack(midpoint_coords, dim=0).to(dtype=dtype, device=device)
        p2_nodes = torch.cat([nodes.to(dtype=dtype, device=device), mids], dim=0)
    else:
        p2_nodes = nodes.to(dtype=dtype, device=device).clone()
