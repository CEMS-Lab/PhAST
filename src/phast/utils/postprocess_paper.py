...
class BenchmarkPostProcessor:
    ...
    def __init__(self, run_dir: str, dpi: int = None, fmt: str = 'png',
        ...
        # (used by scripts/regen_all.py to rebuild figures off the
        # original HPC h5 files without touching the source run_dir).
    ...
    def _raster_plan(self, width=960):
        ...
        elem = self.elements[tri_idx[inside]]
        x1 = x[elem[:, 0]]
        y1 = y[elem[:, 0]]
        x2 = x[elem[:, 1]]
        y2 = y[elem[:, 1]]
...
def main():
    ...
    parser.add_argument('--figures-dir', type=str, default=None,
                        help='Write outputs here instead of <run_dir>/figures/ '
                             '(used by scripts/regen_all.py to fan out over HPC h5s)')
