...
class QuasiStaticSolver:
    ...
    def solve_arc_length_dirichlet(
        ...
        *,
        ...
        spectral_split = energy_split in ('spectral', 'amor', 'star_convex')
        use_autograd_tangent = (
...
class LBFGSSolver:
    ...
    def solve(self, d, f_ext, bc_mask, bc_vals, u_init=None):
        ...
        import warnings
        split = getattr(self.fem.material, 'energy_split', 'isotropic')
        if split in ('spectral', 'amor', 'star_convex'):
            warnings.warn(
...
class MonolithicSolver:
    ...
    def solve(self, u: torch.Tensor, d: torch.Tensor,
        ...
        def closure():
            ...
            psi_plus = fem.compute_psi_plus(u_opt, strain=strain)

            # Compute psi_minus = psi_total - psi_plus
            exx, eyy, gxy = strain
            exy = gxy / 2.0
            tr = exx + eyy
            psi_total = 0.5 * mat.lam * tr ** 2 + mat.mu * (exx ** 2 + eyy ** 2 + 2 * exy ** 2)
            psi_minus = psi_total - psi_plus
