...
class SolverConfig:
    ...
    #     all existing public dynamic benchmarks.
    ...
    #     See reports/B7_dynamic_pipeline_audit_2026-05-08.md (rank #1
    #     non-energy-split candidate cause for the 79us vs COMSOL 33us
    #     branching delay). Subcycling still applies: damage is solved
    #     only on the gated steps, but on those steps the corrector uses
    #     the just-updated d. Costs ~3-5% wall time from the second
    #     internal_force assembly inside the corrector. NOT compatible
...
class StaggeredSolver:
    ...
    def __init__(self, mesh: FEMMesh, material: Material,
        ...
        self.H_elem = torch.zeros(E, dtype=self.dtype, device=self.device)
    ...
    def step_full_arc_length(self, *, lambda_prev: float,
        ...
        if self.config.stagger_criterion == 'am_energy':
            ...
            psi_init = self.fem.compute_psi_plus(self.u, strain=strain_init)
    ...
    def step_compute_driving_force(self, strain=None) -> torch.Tensor:
        ...
        if self.ductile_coupling is not None:
            ...
            psi = self.fem.compute_driving_force(self.u, strain=strain)
    ...
    def _step_full_explicit_fresh_d(self) -> torch.Tensor:
        ...
        See ``reports/B7_dynamic_pipeline_audit_2026-05-08.md`` Sec 11.
        """
        ...
        if not self.config.enable_damage:
            psi = self.fem.compute_psi_plus(u_pred, strain=strain_pred)
    ...
    def step_full(self) -> torch.Tensor:
        ...
        if self.config.solver_type == 'explicit':
            ...
            if not self.config.enable_damage:
                ...
                self._last_relative_residual = float('nan')
                self._last_dt_used = float(self.dt) if self.dt is not None else 0.0
                return self.fem.compute_psi_plus(self.u, strain=strain)
    ...
    def set_damage(self, d_new: torch.Tensor):
        """Inject an externally prepared damage field."""
        self.d = d_new.clone()

    def get_H_nodal(self) -> torch.Tensor:
        """Get current history variable at nodes."""
        return self.H_nodal
    ...
    def pre_strain(self, coupled: bool = False,
        ...
        if not coupled:
            ...
            psi = self.fem.compute_driving_force(self.u)
