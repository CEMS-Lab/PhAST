    ...
    def backward(ctx, grad_output):
        ...
        try:
            ...
            with torch.no_grad():
                ...
                g = g_full
                if need_transfer:
                    g = g.cpu().to(dtype=solver._cg_dtype).to(device=solver._cg_device)
...
class PhaseFieldDamageSolver:
    ...
    def __init__(self, fem: FEMOperators, tol: float = 1e-5,
        ...
        # Precompute float64 mesh data on CG device
        # Route through CPU first — MPS cannot convert to float64 directly
        self._cg_areas = fem.mesh.areas.detach().cpu().to(
            dtype=self._cg_dtype).to(device=self._cg_device)
        self._cg_grad_phi = fem.mesh.grad_phi.detach().cpu().to(
            dtype=self._cg_dtype).to(device=self._cg_device)
        self._cg_elements = fem.mesh.elements.detach().cpu().to(
            device=self._cg_device)
        self._cg_n_nodes = fem.mesh.n_nodes
        if self._native_q4_damage:
            self._cg_quad_N = fem.mesh.quad_N.detach().cpu().to(
                dtype=self._cg_dtype).to(device=self._cg_device)
            self._cg_quad_grad_phi = fem.mesh.quad_grad_phi.detach().cpu().to(
                dtype=self._cg_dtype).to(device=self._cg_device)
            self._cg_quad_wdetJ = fem.mesh.quad_wdetJ.detach().cpu().to(
                dtype=self._cg_dtype).to(device=self._cg_device)
        else:
    ...
    def compute_residual(self, H_input: torch.Tensor,
        ...
        if need_transfer:
            H_cg = H_input.detach().cpu().to(
                dtype=self._cg_dtype).to(device=cg_dev)
            d_cg = d.detach().cpu().to(
                dtype=self._cg_dtype).to(device=cg_dev)
        else:
            H_cg = H_input
            d_cg = d
    ...
    def _prepare_cg(self, H_input, d_prev):
        ...
        if need_transfer:
            H_cg = H_input.detach().cpu().to(
                dtype=self._cg_dtype).to(device=cg_dev)
            d_prev_cg = d_prev.detach().cpu().to(
                dtype=self._cg_dtype).to(device=cg_dev)
            if initial_guess is None:
                d = d_prev_cg.clone()
            else:
                d = initial_guess.detach().cpu().to(
                    dtype=self._cg_dtype).to(device=cg_dev)
        else:
            H_cg = H_input
