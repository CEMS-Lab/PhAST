...
def _scalar_spectral_diag(reaction_coeff, K_local, Gc_l0, elem_flat,
                           n_nodes, dtype, device):
    """Spectral preconditioner: per-element max eigenvalue scaling.

    Instead of using only the diagonal of A (standard Jacobi), this computes
    the maximum eigenvalue of each element's 3x3 local stiffness matrix and
    scatters it to nodes.  The result is a better diagonal approximation that
    accounts for off-diagonal coupling within elements.
    ...
    # Max eigenvalue per element (3x3 symmetric -> torch.linalg.eigvalsh)
    eigvals = torch.linalg.eigvalsh(A_local)  # (E, 3)
    lam_max = eigvals[:, -1]  # largest eigenvalue per element, (E,)
    # Scatter max eigenvalue to nodes (each node gets sum from adjacent elements)
...
def _coarse_solve(r_coarse, A_chol, A_dense, A_sparse=None):
    ...
    if A_sparse is not None:
        ...
        r_np = r_coarse.cpu().to(torch.float64).numpy()
    ...
    if dev_type == 'mps' and A_dense is not None:
        r_cpu = r_coarse.cpu().to(torch.float64)
        A_cpu = A_dense.cpu().to(torch.float64)
...
class ScalarMultigrid:
    ...
    def __init__(self, mesh, aggregation, Gc_l0,
        ...
        self._grad_phi = mesh.grad_phi.detach().cpu().to(
            dtype=dtype).to(device=dev)
        self._areas = mesh.areas.detach().cpu().to(
            dtype=dtype).to(device=dev)
        self._areas_col = self._areas.unsqueeze(1)
        self._elements = mesh.elements.detach().cpu().to(device=dev)
        ...
        self._r_coarse_buf = torch.zeros(
            aggregation.n_coarse, dtype=dtype, device=dev)
    ...
    def vcycle(self, r_fine, reaction_coeff):
        ...
        n_c = self.agg.n_coarse
        if self._r_coarse_buf.shape[0] != n_c:
            self._r_coarse_buf = torch.zeros(
                n_c, dtype=self._dtype, device=self._device)
        r_coarse = self._r_coarse_buf
        r_coarse.zero_()
        r_coarse.scatter_add_(0, agg_id, r_smooth)  # R @ r

        # --- Coarse solve ---
        A_sp = getattr(self, '_A_coarse_sparse', None)
        e_coarse = _coarse_solve(r_coarse, self._A_coarse_chol, self._A_coarse, A_sp)
...
class AmgXPreconditioner:
    ...
    def __init__(self, mesh, Gc_l0, device=None, dtype=torch.float64):
        ...
        gp_cpu = mesh.grad_phi.detach().cpu().to(dtype=dtype)
        areas_col_cpu = mesh.areas.detach().cpu().to(dtype=dtype).unsqueeze(1)
    ...
    def vcycle(self, r_fine, reaction_coeff=None):
        ...
        r_np = r_fine.detach().cpu().to(torch.float64).numpy()
...
class AMGPreconditioner:
    ...
    def _estimate_spectral_radius(self, reaction_coeff, n_iters=10):
...
class VectorMultigrid:
    ...
    def _build_B_matrices(self, grad_phi):
        ...
        B = torch.zeros(E_count, 3, 6, dtype=self._dtype,
                        device=self._device)
        for a in range(3):
            B[:, 0, 2 * a] = grad_phi[:, a, 0]      # exx from u_ax
            B[:, 1, 2 * a + 1] = grad_phi[:, a, 1]  # eyy from u_ay
            B[:, 2, 2 * a] = grad_phi[:, a, 1]      # gxy from u_ax
            B[:, 2, 2 * a + 1] = grad_phi[:, a, 0]  # gxy from u_ay
        return B
