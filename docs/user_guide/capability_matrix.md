...
| Heterogeneous elastic fields `E(x)` | Production | Per-element fields support inclusions, weak/strong bands, and forward microstructure-style studies. |
| Heterogeneous fracture fields `Gc(x)` | Production | Per-element fields support weak zones and forward microstructure-style studies. |
...
| Spectral / Amor / isotropic energy splits | Production | Available through `material.energy_split` for brittle phase-field workflows. |
...
| Sparse quasi-static J2 elastoplasticity | Beta | Per-element state, return mapping, commit/rollback, internal force, sparse `QuasiStaticSolver` dispatch, and plastic-work accounting are available through the plasticity API and validation example. Public examples exercise the path at validation scale; large-mesh backend promotion remains gated. |
...
| Capability | Status | Customer-safe statement |
|---|---|---|
| YAML problem definition | Production | Canonical entry point: `python -m phast run configs/benchmarks/<family>/<name>.yaml`. |
...
| Capability | Status | Customer-safe statement |
|---|---|---|
| Zarr trajectory stores | Production for solver trajectories | Optional chunked trajectory output for post-processing, restart-style inspection, and large forward-run artifacts. |
| HDF5 snapshots | Legacy compatibility | Supported for older benchmark post-processing and import/export bridges; prefer the documented CSV/plot/Zarr outputs for new forward runs. |
...
> validation example plus sensitivity study,
