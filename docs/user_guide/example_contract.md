...
`docs/STANDARD_OUTPUTS.md`, `docs/visualisation_requirements.md`,
`docs/visualization-output.md`, `examples/PUBLIC_EXAMPLES_CONTRACT.yaml`, and
`configs/benchmarks/plasticity_interface/reproducibility_contracts.yaml`.
Those files remain compatibility or machine-readable manifests, but this page
is the human-facing source of truth.
...
raw run tree or private scratch archive.
...
examples/<family>/<case>/
  ...
  zarr_manifest.json           # required for every trajectory store
...
Do not commit raw HPC result folders, private COMSOL binaries, full scratch
paths, large trajectory stores, or archaeology from exploratory runs. Small
tutorial Zarr stores are allowed when they are generated by the example runner,
listed in `zarr_manifest.json`, and pass artifact-size checks. Heavy
`training_data.zarr/`, legacy `training_data.h5`, VTU/PV time series, and
large raw trajectory archives belong in private storage or a named release
artifact, with `zarr_manifest.json` recording how to retrieve or regenerate
them. A compact `training_data.zarr/` may be checked in only when it is
explicitly listed in a promoted retained-result manifest, passes artifact
hygiene, and backs the documented `phast.load_result(path).field(...)`
inspection surface.
...
promotion date. Do not expose private absolute scratch paths in public docs.
...
- Tutorial-ready examples must provide `training_data.zarr/` when the store is
  small enough for the development repository. If the full trajectory is too
  large for git, provide `zarr_manifest.json` with the storage location,
  expected schema, regeneration command, and issue/HPC provenance.
...
- `.pv`/PyVista-zstd is an optional fast-path for large visualization output;
  see `docs/visualization-output.md`.

Do not commit raw heavy trajectory stores in public examples. Promote a
lightweight summary, manifest, plots, and a documented private/archive
location instead. Small tutorial Zarr stores are acceptable only when they are
generated from the checked-in deck and remain comfortably below the repository
artifact-size gate.
...
run or array task. Public docs should describe the relative structure, not a
private absolute scratch path.
...
private user paths, secrets, or credentials.
...
Keep raw heavy trajectories, private scratch paths, and intermediate solver
...
- release export checks to exclude private archives and heavy raw stores;
...
`docs/STANDARD_OUTPUTS.md`, `docs/visualisation_requirements.md`,
`docs/visualization-output.md`, `examples/README.md`, and the public docs
index.
...
- [ ] heavy/private files are excluded;
