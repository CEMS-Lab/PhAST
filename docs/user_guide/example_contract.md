...
|---|---|---|
| YAML-first | `python -m phast run <config.yaml>` | Customer-runnable input deck with a curated result bundle. |
...
`training_data.zarr/`, legacy `training_data.h5`, VTU/PV time series, and raw
trajectory archives belong in local rerun output directories or named external
release artifacts. The public repository should retain the lightweight YAML,
Python runner, CSV, metadata, image, and animation artifacts needed to
understand and rerun the case.
...
- the response/evolution animation artifacts;
...
If a script-contract workflow becomes customer-standard, promote it by adding
...
- `tutorial_ready: true`: the leaf folder has the complete tutorial bundle:
  ...
  setup visual, final field plots, response/evolution animations, run
...
- Every tutorial-ready example must include `response_evolution.mp4`, even
...
- "production" for tested customer-facing workflows;
