...
The rule is simple: a curated example must be runnable, inspectable,
...
| Diagnostic | Case-specific | Developer evidence only; not a gallery example until deliberately curated. |
...
Each curated run should write artifacts that can be inspected by users and by
release checks without reverse-engineering a solver scratch directory.

| Artifact | Producer | Primary consumer | Public contract |
|---|---|---|---|
| `run_manifest.json` | YAML runner, curated scripts | `phast.load_result`, release review, example README | Required for curated examples. |
...
Every curated example `README.md` must contain:
...
- `tutorial_ready: true`: the leaf folder has the complete tutorial bundle:
  config.yaml, `run_fluent.py` or equivalent fluent authoring companion,
...
- `tutorial_ready: false`: the entry is useful but incomplete as a full
  ...
  connector support, beta-script curation, or trajectory externalization.

Do not set `tutorial_ready: true` for an example unless its artifacts are
generated by the solver or postprocessor and recorded in the public example
contract. Do not create placeholder Zarr files or hand-made animations to
satisfy the flag.

## Inputs and provenance

A curated example must make the setup inspectable before the solver runs.
...
|---|---|---|
| `run_manifest.json` | Every curated example | command, git commit, script/config, result path, issue/review notes where applicable, and artifact list. |
...
| `visual_manifest.json` | Any curated visual output | file list, kind, source data, dimensions, generation command, and claim role. |
...
curation date. Do not expose absolute scratch paths in public docs.
...
| `timing_per_step.csv` | curated performance or HPC examples | step, wall time, solver components, output time where available |
...
Every curated visual should use physically meaningful color limits and paper
...
Use the repository paper style for curated figures:
...
Every curated example README should include a short inspection snippet:
...
Curated examples must be runnable locally first, but HPC is the expected path
...
list config path, output key, device request, expected wall time, and curation
...
HPC-curated outputs must record:
...
- result folder key;
- curation command/date and copied artifact list.

The separate `lab-infra` repository is the operational source for current
cluster paths, modules, queue aliases, and curation conventions. Public docs
...
Curation is not complete until tests or drift checks cover the contract:
...
At minimum, the example contract page must remain linked from
`examples/README.md` and the public docs index.

## Curation checklist

Before declaring an example curated:
...
- [ ] issue or review thread records the curation evidence.
