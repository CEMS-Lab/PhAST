...
- **Principal stress criterion production path** — scaffold exists, but
  ...
  fracture workflows.
- **Plane-stress strain-spectral parity** — PhAST's plane-stress `spectral`
  path is a reduced 2D in-plane projection. It is not a fully condensed 3D
  plane-stress spectral decomposition with damage-dependent out-of-plane
...
| Branching | ~33 µs in the COMSOL application; Ren 2019 reports 68.2-70.1 µs for the published replication window | Default B7 comparator uses the Ren/Borden-style timing window; COMSOL timing parity is retained as diagnostic context |
...
Full benchmark parameter tables will be published with the Paper 1 archive;
the public solver repository keeps only the runnable YAML/configuration surface
needed for the staged release.
