...
submitting to a workstation or HPC queue. This block is deliberately
free-form: it records what the run is expected to reproduce, not another set
of solver controls. `explain-config` prints it, and the resolved config and
run lockfile preserve it for later review.
...
- `training_data.zarr` or a run-level `.zarr` store -- optional chunked
  trajectory output when configured
...
| Use case | Template |
|----------|----------|
| Dynamic SENT branching | `configs/benchmarks/dynamic/B1_branching_glass.yaml` |
...
Once a config runs cleanly, head to the [example gallery](../example-gallery.md)
for validated workflows and outputs to compare against.
