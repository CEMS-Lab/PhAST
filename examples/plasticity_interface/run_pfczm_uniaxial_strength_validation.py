...
def _render_damage_frame(nodes: np.ndarray, elements: np.ndarray,
    ...
    tpc = ax.tripcolor(
        ...
        shading="gouraud", cmap="magma", vmin=0.0, vmax=1.0)
...
def _write_run_metadata(output_dir: Path, case: PFCZMCase,
    ...
    (output_dir / "run_lockfile.json").write_text(json.dumps(common, indent=2) + "\n")
...
def run_validation(output_dir: Path, *,
    ...
    for l0 in l0_values:
        ...
        for step, sigma in enumerate(trial_stresses):
            ...
            if l0 == l0_values[-1] and step % max(1, case.n_steps // 10) == 0:
                frames.append(_render_damage_frame(
                    nodes, elements, d.detach().cpu().numpy(),
    ...
    _write_csv(output_dir / "timing_per_step.csv", [
        {"l0": r["l0"], "step": r["step"], "elapsed_s": 0.0}
        for r in rows
    ])
    ...
    manifest_paths = [
        ...
        "mesh.msh", "results.csv", "history.csv", "energy.csv",
        "solver_telemetry.csv", "timing_per_step.csv", "damage_final.png",
        "load_displacement.png", "damage_history.png", "energy_split.png",
        "convergence.png", "mesh_deformed.png", "damage_evolution.gif",
