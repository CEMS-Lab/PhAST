...
STAGGER_DAMAGE_DELTA_TOL = 2.0e-3
DAMAGE_RESIDUAL_TOL = 5.0e-4
...
def run_benchmark(output_dir: Path, *, n_steps: int = 9,
    ...
    for step, opening in enumerate(openings):
        ...
        for stagger_iter in range(1, max_stagger_iter + 1):
            ...
            damage_solver_hit_max_iter = (
                int(getattr(damage_solver, "last_iter", -1))
                >= int(getattr(damage_solver, "max_iter", 0))
            )
            if (
                damage_delta < STAGGER_DAMAGE_DELTA_TOL
                and damage_residual_norm < DAMAGE_RESIDUAL_TOL
                and not damage_solver_hit_max_iter
            ):
        ...
        if not stagger_converged:
            raise RuntimeError(
                f"Coupled stagger loop failed at step {step}: "
                f"damage_delta={damage_delta:.3e}, "
                f"damage_residual={damage_residual_norm:.3e}, "
                f"damage_hit_max_iter={damage_solver_hit_max_iter}")
        ...
        free_damage = d[~pf_mask]
        nonpinned_matrix_damage_max = (
            float(free_damage.max().item()) if free_damage.numel() else 0.0)
        nonpinned_matrix_damage_mean = (
            float(free_damage.mean().item()) if free_damage.numel() else 0.0)
        ...
        row = {
            ...
            "stagger_converged": bool(stagger_converged),
            "stagger_damage_delta": float(damage_delta),
            "damage_pcg_iterations": int(getattr(damage_solver, "last_iter", -1)),
            "damage_solver_hit_max_iter": bool(damage_solver_hit_max_iter),
            ...
            "nonpinned_matrix_damage_mean": nonpinned_matrix_damage_mean,
            "nonpinned_matrix_damage_max": nonpinned_matrix_damage_max,
    ...
    summary = {
        ...
        "all_stagger_steps_converged": all(row["stagger_converged"] for row in rows),
        "all_damage_solves_converged": not any(
            row["damage_solver_hit_max_iter"] for row in rows),
        ...
        "stagger_damage_delta_tolerance": STAGGER_DAMAGE_DELTA_TOL,
        "damage_residual_tolerance": DAMAGE_RESIDUAL_TOL,
        "final_matrix_damage_max": float(rows[-1]["matrix_damage_max"]),
        "final_matrix_damage_mean": float(rows[-1]["matrix_damage_mean"]),
        "final_nonpinned_matrix_damage_max": float(
            rows[-1]["nonpinned_matrix_damage_max"]),
        "final_nonpinned_matrix_damage_mean": float(
            rows[-1]["nonpinned_matrix_damage_mean"]),
        "max_nonpinned_matrix_damage_max": float(
            max(row["nonpinned_matrix_damage_max"] for row in rows)),
        ...
        "validation_passed": bool(
            ...
            and all(row["stagger_converged"] for row in rows)
            and not any(row["damage_solver_hit_max_iter"] for row in rows)
            and max(row["mechanics_residual"] for row in rows) < 1.0e-7
            and max(row["damage_residual_norm"] for row in rows) < DAMAGE_RESIDUAL_TOL
            and max(row["stagger_damage_delta"] for row in rows) < STAGGER_DAMAGE_DELTA_TOL
            and rows[-1]["matrix_damage_max"] >= 1.0 - 1.0e-12
            and max(row["nonpinned_matrix_damage_max"] for row in rows) > 1.0e-4
