...
The compact smoke scripts write:
...
On machines with PETSc/MUMPS, `backend=auto` should resolve to MUMPS after the
runtime smoke check and match the SciPy baseline within the recorded tolerance.
When cuDSS/nvmath is unavailable, the harness records a clean fallback reason
instead of treating the request as validated. cuDSS remains a separate open
promotion gate until a functional GPU sparse-direct environment resolves
`backend='cudss'` without fallback.
...
It writes the same reproducibility bundle plus `backend_promotion.csv` and one
child bundle per requested backend. The SciPy path is the portable baseline;
PETSc/MUMPS and cuDSS depend on the active backend environment and remain
target-machine promotion checks. Optional Slurm launchers live under
`scripts/slurm/plasticity/` for teams running the same promotion matrix on a
cluster.
