...
def _cohesive_damage_to_nodes(
    ...
    for ce, value in zip(op.cohesives, damage_profile):
        for node in (*ce.nodes_top, *ce.nodes_bottom):
            nodal[node] += float(value)
            counts[node] += 1.0
    return nodal / np.maximum(counts, 1.0)


def _render_damage_frame(
    nodes: np.ndarray,
    elements: np.ndarray,
    values: np.ndarray,
    title: str,
    ...
    ax.triplot(nodes[:, 0], nodes[:, 1], elements, color="0.75", linewidth=0.25)
    ax.set_aspect("equal")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title(title)
    fig.colorbar(tpc, ax=ax, shrink=0.8).set_label("cohesive damage")
    fig.canvas.draw()
...
def _write_initial_conditions_plot(
    output_dir: Path,
    *,
    nodes: np.ndarray,
    elements: np.ndarray,
    ...
    fig, ax = plt.subplots(figsize=(8.0, 4.2), constrained_layout=True)
    ax.triplot(nodes[:, 0], nodes[:, 1], elements, color="0.45", linewidth=0.35)
    ax.axvspan(0.0, geom.initial_crack_length, color="tab:red", alpha=0.14,
               label="initial precrack")
    ax.axhline(0.0, color="tab:blue", linewidth=1.4, label="bonded interface")
    ax.axvline(geom.length, color="black", linestyle="--", linewidth=1.0,
               label="clamped end")
    ax.set_aspect("equal")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Structural DCB initial mesh, precrack, and interface")
...
def _write_final_damage_plot(
    ...
    *,
    ...
    path = output_dir / "cohesive_damage_final.png"
    values = _cohesive_damage_to_nodes(nodes.shape[0], op, damage_profile)
    fig, ax = plt.subplots(figsize=(8.0, 4.2), constrained_layout=True)
    tpc = ax.tripcolor(
        nodes[:, 0], nodes[:, 1], elements, values,
        shading="gouraud", cmap="Reds", vmin=0.0, vmax=1.0)
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Final retained cohesive damage field")
    fig.colorbar(tpc, ax=ax, shrink=0.8).set_label("cohesive damage")
    fig.savefig(path, dpi=170)
    plt.close(fig)
    return path
...
def run_benchmark(
    ...
    *,
    ...
    openings = np.linspace(0.0, max_opening, n_load_steps)
    ...
    for step, opening in enumerate(openings):
        ...
        damage_nodal = _cohesive_damage_to_nodes(mesh.n_nodes, op, damage_profile)
        if step in {0, len(openings) - 1} or step % max(1, len(openings) // 5) == 0:
            snapshots.append((
                int(step),
                {
                    "displacement": u.detach().cpu().numpy(),
                    "cohesive_damage": damage_nodal,
                },
            ))
            frames.append(_render_damage_frame(
                nodes, mesh.elements.cpu().numpy(), damage_nodal,
                f"Structural DCB cohesive damage, step {step}"))
    ...
    initial_png = _write_initial_conditions_plot(
        output_dir, nodes=nodes, elements=mesh.elements.cpu().numpy(), geom=geom)
    damage_final_png = _write_final_damage_plot(
        output_dir, nodes=nodes, elements=mesh.elements.cpu().numpy(),
        op=op, damage_profile=damage_profile)
    animation_path = output_dir / "damage_evolution.gif"
    _write_animation(animation_path, frames)
    image_paths.extend([initial_png, damage_final_png, animation_path])
    write_csv_rows(output_dir / "history.csv", rows)
    write_csv_rows(output_dir / "energy.csv", [
        {
            "step": row["step"],
            "opening": row["opening"],
            "bulk_elastic_energy": row["bulk_elastic_energy"],
            "cohesive_dissipated_energy": row["cohesive_dissipated_energy"],
            "external_work": row["external_work"],
            "energy_balance_gap": row["energy_balance_gap"],
        }
        for row in rows
    ])
    ...
    write_zarr_trajectory(
        ...
        snapshots=snapshots,
        metadata={
            "validation_id": "structural_dcb_cohesive",
            "field_source": "solver_coupled_cohesive_interface",
        },
    )
    ...
    merge_run_manifest_artifacts(output_dir, [path.name for path in artifacts])
