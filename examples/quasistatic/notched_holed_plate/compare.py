...
REF = HERE / "reference_solutions" / "comsol_load_displacement.csv"
# This benchmark currently has no long-lived in-repo canonical run package.
# If this changes later, add the accepted run directory here to enable
# no-argument compare runs.
CANONICAL_REFERENCE_RUNS: list[Path] = []
...
def load_reference() -> tuple[np.ndarray, np.ndarray]:
    ...
    # = 2 x per-pin displacement); see run.py:282 `--u_max` help string
    # ("matches COMSOL para * 2 mm") and the module docstring above.
...
def main():
    ...
    if run_dir is None or not (
        ...
        print(f"FAIL: no run output CSV under {HERE}; run run.py first.",
              file=sys.stderr)
        sys.exit(2)
