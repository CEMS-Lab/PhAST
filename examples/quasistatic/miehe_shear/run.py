#!/usr/bin/env python
"""
Miehe SENS (Single-Edge-Notch Shear) — Full benchmark run.
...
Usage:
    python -u examples/quasistatic/miehe_shear/run.py
    python -u examples/quasistatic/miehe_shear/run.py --all_outputs
    python -u examples/quasistatic/miehe_shear/run.py --trajectory --gif
"""
...
# ------------------------------------------------------------------ #
# Problem parameters (PhaseFieldX Example 1712)
# ------------------------------------------------------------------ #
PROBLEM = {
    'name': 'Miehe SENS (Single-Edge-Notch Shear)',
    'reference': 'Miehe et al. (2010), PhaseFieldX Example 1712',
    'L': 1.0,            # plate side length [mm]
    'a': 0.5,            # notch length [mm]
    'E': 210000.0,       # Young modulus [MPa]
    'nu': 0.3,
    'Gc': 2.7,           # fracture toughness [N/mm]
    'l0': 0.06,          # regularization length [mm] (larger for shear)
    'energy_split': 'spectral',
    'pf_model': 'AT2',
    'eta_residual': 0.0,         # PhaseFieldX: k=0 (no residual stiffness)
    # Loading: single linear ramp (PhaseFieldX uses no slow phase for shear)
    'dt': 1e-4,          # [mm/step]
}
...
def setup_bcs(mesh):
    ...
    - Bottom: fixed x, fixed y
    - Top: prescribed u_x=1 (shear, scaled by load_factor), fixed y
    - Left/Right: fixed y (u_y=0), free x
    """
    bcs = BoundaryConditions(mesh.n_nodes, mesh.device, mesh.dtype)
    bcs.fix(mesh.node_sets['bottom'], 0)
    bcs.fix(mesh.node_sets['bottom'], 1)
    bcs.add(mesh.node_sets['top'], 0, 1.0)  # unit disp, scaled by load_factor
    bcs.fix(mesh.node_sets['top'], 1)
    # Left/right: fix y to prevent rigid body rotation
...
def build_loading_schedule(n_steps):
    """PhaseFieldX SENS: single linear ramp at dt=1e-4 mm/step."""
    dt = PROBLEM['dt']
    return [dt * t for t in range(1, n_steps + 1)]
...
def main():
    ...
    parser.add_argument('--gif_fields', type=str,
                        default='damage,stress_xy,max_principal_stress')
    ...
    if args.output_dir is None:
        ...
        args.output_dir = os.path.join(
            base_dir,
            generate_run_tag(ctx.device, extra=f'sens_{args.at_mode}'))
    ...
    # ---- Material (spectral split for shear) ----
    l0 = args.l0
    ...
    mat = create_material('miehe_shear', **mat_kwargs)
    print(mat)
    ...
    crack_step = None
    crack_stage_targets = [0.0090, 0.0110, 0.0134]
    ...
    while pending_displacements:
        ...
        # Reaction force (PhaseFieldX plots -bottom Rx for the shear benchmark)
        ...
        if accepted_step % args.print_every == 0:
            print(f"  Step {accepted_step:3d}: u_x={disp_val:.6f}, R_x={-R/1000:.4f} kN, "
    ...
    if args.plots:
        ...
        ref_dir = os.path.join(os.path.dirname(__file__), '..', '..',
                               'reference_solutions')
        ref_file = os.path.join(ref_dir, 'miehe_solution_shear.csv')
        ...
        ax.set_title(f'SENS Validation — l0={l0}, h={args.h_crack}')
        ax.legend()
        ax.grid(True, alpha=0.3)
        crack_length = 0.7  # diagonal crack ~0.7mm (approximate)
        ...
        if crack_stage_snapshots:
            ...
            fig3.suptitle('SENS crack-path propagation stages', fontsize=12)
            fig3.tight_layout()
