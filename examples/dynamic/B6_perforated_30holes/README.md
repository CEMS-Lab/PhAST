...
Selected PMMA perforated-plate dynamic fracture example based on Bleyer et al. (2017). This public B6 folder retains the 30-hole crack-hole interaction case and lightweight artifacts from the reference run.
...
- Hole layout: 30 holes with 0.9 mm spacing, spanning x = 5.0 mm to x = 31.1 mm; hole diameter is 0.4 mm.
...
The YAML configuration is the canonical public input for this example. The retained reference run used 195,035 nodes, 387,937 elements, 37,141 explicit steps, and an A100 80 GB GPU. Do not regenerate the full benchmark during lightweight contract checks.
...
```bash
gmsh examples/dynamic/B6_perforated_30holes/mesh.geo \
  -2 -format msh2 \
  -o examples/dynamic/B6_perforated_30holes/mesh.msh
```
...
|---|---|
| <img src="initial_conditions.png" alt="B6 Perforated 30-Hole Plate initial conditions" width="360"> | <img src="damage_evolution.gif" alt="B6 Perforated 30-Hole Plate damage evolution" width="360"> |

| Quantity | Reference evidence | Status |
| --- | --- | --- |
| Recorded run | Retained B6 reference run; 195,035 nodes, 387,937 elements, 37,141 explicit steps | PRESENT |
| Hole layout | 30 holes with 0.9 mm spacing, spanning x = 5.0 mm to x = 31.1 mm | DOCUMENTED |
