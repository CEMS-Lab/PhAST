...
Curated PMMA dynamic fracture variant with a single hole near the crack path from the Bleyer et al. (2017) perforated-plate family. This public B6 folder retains lightweight artifacts from the reference run.
...
- 32 mm x 16 mm PMMA SENT plate with one hole at the near-hole placement and public B6 naming.
- Hole layout: 1 hole at x = 5.0 mm with vertical offset 0.6 mm; hole diameter is 0.4 mm.
...
The YAML configuration is the canonical public input for this example. The retained reference run used 50,201 nodes, 99,811 elements, 47,900 explicit steps, and an A100 80 GB GPU. Do not regenerate the full benchmark during lightweight contract checks.
...
python -m phast run examples/dynamic/B6_perforated_1hole_near/config.yaml \
  --output_dir examples/dynamic/B6_perforated_1hole_near/run_local
```
...
python -m phast run examples/dynamic/B6_perforated_1hole_near/config.yaml \
  --num_steps 5 \
  --output_dir examples/dynamic/B6_perforated_1hole_near/run_quick
```
...
```bash
gmsh examples/dynamic/B6_perforated_1hole_near/mesh.geo \
  -2 -format msh2 \
  -o examples/dynamic/B6_perforated_1hole_near/mesh.msh
```
...
```bash
python examples/dynamic/B6_perforated_1hole_near/run_fluent.py \
  --output-dir examples/dynamic/B6_perforated_1hole_near/run_fluent
```
...
python examples/dynamic/B6_perforated_1hole_near/run_fluent.py \
  --num-steps 5 \
  --output-dir examples/dynamic/B6_perforated_1hole_near/run_fluent_quick
```
...
|---|---|
| <img src="initial_conditions.png" alt="B6 Perforated 1-Hole Near Plate initial conditions" width="360"> | <img src="damage_evolution.gif" alt="B6 Perforated 1-Hole Near Plate damage evolution" width="360"> |

| Quantity | Reference evidence | Status |
| --- | --- | --- |
| Recorded run | Retained B6 reference run; 50,201 nodes, 99,811 elements, 47,900 explicit steps | PRESENT |
| Hole layout | 1 hole at x = 5.0 mm with vertical offset 0.6 mm | DOCUMENTED |
