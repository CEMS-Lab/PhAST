"""Run the B6 perforated 1-hole far plate example without a YAML configuration."""
...
def build_problem(num_steps: int | None = None) -> phast.Problem:
    problem = phast.Problem("Perforated Plate — 1 hole far")
    problem.config.example = "phast.examples.dynamic_perforated_plate.run"
    problem.config.reference = "Bleyer et al. (2017), Sec 4.3"
    ...
    problem.config.output = OutputConfig(trajectory=True, h5=True, h5_every=20, fast=True, print_every=100)
    return _apply_num_steps(problem, num_steps)


def main() -> int:
    parser = argparse.ArgumentParser(description="Run B6 1-hole far perforated plate through the Python API.")
    parser.add_argument("--output-dir", default="examples/dynamic/B6_perforated_1hole_far/run_fluent")
